...
The final controlled validation proved this path with real AWS and GitHub services:

```text
GitHub Actions → OIDC → ECR digest → promotion PR → main → Argo CD
                                                        ↓
Internet → ALB → Service → CareFlow pods → private RDS PostgreSQL
```
