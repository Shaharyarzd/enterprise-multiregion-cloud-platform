...
| ALB HTTP response/target health | Public traffic reaches healthy IP targets and the application/data path | Shows practical edge completion without overstating TLS | Conditionally | Redact account/zone IDs, private identifiers and unrelated browser tabs |
| ALB HTTPS response/certificate view | Public traffic uses redirect plus a hostname-valid certificate | Shows trusted-TLS completion; currently PENDING | Conditionally | Use a controlled portfolio hostname; remove account/zone IDs and unrelated browser tabs |
