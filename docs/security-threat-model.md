...
- **Workloads:** restricted Pod Security, tokenless ServiceAccount, non-root execution, read-only root filesystem, dropped capabilities, seccomp, default-deny policies and pod branch ENIs were exercised. Broader runtime detection remains absent.
- **Database:** private placement, encrypted storage, forced server TLS, External Secrets delivery, application-side hostname validation and a pod-specific security group executed successfully. Managed rotation is configured but recovery remains PENDING because the rotation request was rejected before changing credentials. The master principal remains too privileged for a persistent production workload; split migrator/runtime roles are documented next work.
- **Supply chain:** third-party Actions are SHA-pinned and Trivy gates source/IaC/image findings. GitHub OIDC published to immutable ECR and promotion changed only a digest through a pull request. Kyverno enforced the digest policy, though its Argo applications retained convergence drift. SBOM retention, provenance, signing and verified admission remain planned.
...
- **Ingress:** ALB IP targets and dependency health checking executed with two healthy targets over HTTP. Optional ACM input, redirect and a modern TLS policy remain unexecuted without a controlled domain. WAF and durable ALB access logs are deliberately deferred pending production threat/cost justification.
